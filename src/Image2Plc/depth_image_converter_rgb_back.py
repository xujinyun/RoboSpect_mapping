...
for x in range(count):
    ...
    for i in range(img_height):
        for j in range(img_width):
            ...
            rgb = (cur_r << 16) | (cur_g << 8) | cur_b
            # rgb = 0
            

            point_list += np.float32(cur_x).tobytes() + np.float32(cur_y).tobytes() + np.float32(cur_depth).tobytes() + np.uint32(rgb).tobytes()
            # print(point_list)
            # exit(0)
            print(i)
        break
    ...
    new_msg.height = 1
    new_msg.width = int(len(point_list) / 16)
    ...
    new_msg.data = point_list # np.asarray(points_np, np.float32).tostring()
