...
void LaserProcessingClass::featureExtraction(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pc_in, pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pc_out_edge, pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pc_out_surf)
{
    ...
    // std::cout << "pc_in->points.size(): " << pc_in->points.size() << std::endl;
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        // if(pc_in->points[i].z<-1.0||pc_in->points[i].z>1.0||pc_in->points[i].x<0.5 )
        //     continue;
        if (isnan(pc_in->points[i].x) || isnan(pc_in->points[i].y) || isnan(pc_in->points[i].z))
            continue;
        ...
        // std::cout << "angle: " << angle << " last_angle: " << last_angle << std::endl;

        if (fabs(angle - last_angle) > 0.01)     //0.05
        {

            // std::cout << " " << pc_in->points[i].x << " " << pc_in->points[i].y << " " << pc_in->points[i].z << std::endl;
            if (count > 100) //100
            {
    ...
    ROS_WARN("total points array %d", laserCloudScans.size());
    // open the file and save total points array to file
    std::ofstream outfile;
    outfile.open("/home/jinyun/L515SLAM/src/ssl_slam/src/total_points_array.txt", std::ios::app);
    outfile  << laserCloudScans.size() << std::endl;
    outfile.close();
